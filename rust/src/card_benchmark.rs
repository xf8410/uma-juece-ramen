...
#[derive(Deserialize,Default)]#[serde(default,rename_all="camelCase")]
struct RawCard{card_id:u32,card_name:String,rarity:i32,card_type:usize,card_value:Vec<RawValue>}
#[derive(Deserialize,Default)]#[serde(default,rename_all="camelCase")]
struct RawValue{bonus:Vec<i32>,initial_bonus:Vec<i32>,hint_bonus:Vec<i32>,you_qing:f64,gan_jing:f64,xun_lian:f64,initial_ji_ban:f64,hint_prob_increase:f64,de_yi_lv:f64,sai_hou:f64,wiz_vital_bonus:f64,fail_rate_drop:f64,vital_cost_drop:f64}
...
fn load_cards(path:&str,min_id:u32)->anyhow::Result<Vec<Card>>{
 ...
 Ok(raw.into_values().filter_map(|c|{
  ...
  let proxy=v.xun_lian*3.0+v.you_qing*2.0+v.de_yi_lv*.35+v.gan_jing*.25+v.initial_ji_ban*.45+v.sai_hou*1.5+v.wiz_vital_bonus*8.0+v.fail_rate_drop+v.vital_cost_drop+sum(&v.bonus)*16.0+sum(&v.initial_bonus)*.4+sum(&v.hint_bonus)*.15+v.hint_prob_increase*.1;
...
fn archetype(counts:[usize;5],pools:&[Vec<Card>;5])->Option<Deck>{
 if counts.iter().sum::<usize>()!=5{return None}
 let mut used=HashSet::new();let mut ids=Vec::with_capacity(6);
 for t in 0..5{for c in pools[t].iter(){if ids.iter().filter(|id|pools[t].iter().any(|x|x.id==**id)).count()>=counts[t]{break}if used.insert(c.id){ids.push(c.id)}}}
 if ids.len()!=5{return None}ids.push(FRIEND);
 let cards:[u32;6]=ids.try_into().ok()?;
 Some(Deck{name:composition_name(counts),cards})
}
...
fn build_decks(cards:&[Card])->Vec<Deck>{
 ...
 // 系统枚举：五种普通卡数量合计为5；单类型最多3张。允许某些类型为0。
 // 因此会覆盖用户指出的 0速1耐1力0根3智+1友，也覆盖纯力/根偏重编成。
 for speed in 0..=3{for stamina in 0..=3{for power in 0..=3{for guts in 0..=3{for wiz in 0..=3{
  let counts=[speed,stamina,power,guts,wiz];
  if counts.iter().sum::<usize>()==5{if let Some(d)=archetype(counts,&pools){decks.push(d)}}
 }}}}}
 // 力卡、根卡逐张测试：只替换基准中的一个速度槽，总数仍是6。
 ...
 decks.retain(|d|d.cards.len()==6&&d.cards[5]==FRIEND&&d.cards[..5].iter().all(|id|*id!=FRIEND)&&seen.insert(d.cards));
 decks
}

fn simulate(deck:&Deck,n:usize,strategy:&RamenStrategy)->ResultRow{
 assert_eq!(deck.cards.len(),6);assert_eq!(deck.cards[5],FRIEND);
 let results:Vec<(i32,i32,bool)>=(0..n).into_par_iter().filter_map(|_|{let mut g=RamenGame::newgame(TEST_UMA,&deck.cards,INHERIT).ok()?;let mut rng=rand::rngs::StdRng::from_os_rng();g.run_full_game(strategy,&mut rng).ok()?;Some((g.uma().calc_score(),g.uma().total_pt(),g.ramen.rmj_results.iter().take(3).all(|&x|x)))}).collect();
 let ok=results.len();let mut s:Vec<_>=results.iter().map(|x|x.0).collect();s.sort_unstable();
 let mean=if ok==0{0.0}else{s.iter().map(|&x|x as f64).sum::<f64>()/ok as f64};
 let pt=if ok==0{0.0}else{results.iter().map(|x|x.1 as f64).sum::<f64>()/ok as f64};
 let rmj=if ok==0{0.0}else{results.iter().filter(|x|x.2).count()as f64*100.0/ok as f64};
 ResultRow{name:deck.name.clone(),cards:deck.cards,ok,failed:n-ok,mean,median:s.get(ok/2).copied().unwrap_or(0),p90:s.get((ok*9/10).min(ok.saturating_sub(1))).copied().unwrap_or(0),max:s.last().copied().unwrap_or(0),pt_mean:pt,rmj_all:rmj}
}
fn main()->anyhow::Result<()>{
 let args:Vec<_>=env::args().collect();let n=args.get(1).and_then(|x|x.parse().ok()).unwrap_or(300);let min_id=env::var("MIN_CARD_ID").ok().and_then(|x|x.parse().ok()).unwrap_or(DEFAULT_MIN_CARD_ID);
 ...
 println!("# 拉面杯类型构成 + 力根逐卡模拟\n\n- 每套严格6张：5张普通卡 + 固定友人 {}\n- 系统枚举五种类型合计5张、每种最多3张，允许某些类型为0\n- 包含 3智1力1耐+1友；另逐张测试近期力卡、根卡\n- 仅近期满破SSR（card_id >= {}），每套{}局，共{}套\n",FRIEND,min_id,n,decks.len());
 let start=Instant::now();let mut rows:Vec<_>=decks.iter().map(|d|{eprintln!("模拟 {} {:?}",d.name,d.cards);simulate(d,n,&strategy)}).collect();rows.sort_by(|a,b|b.mean.total_cmp(&a.mean));
 println!("|排名|卡组|均分|中位|P90|最高|均PT|RMJ全通|成功/失败|\n|---:|---|---:|---:|---:|---:|---:|---:|---:|");for(i,r)in rows.iter().enumerate(){println!("|{}|{}|{:.0}|{}|{}|{}|{:.0}|{:.1}%|{}/{}|",i+1,r.name,r.mean,r.median,r.p90,r.max,r.pt_mean,r.rmj_all,r.ok,r.failed)}
