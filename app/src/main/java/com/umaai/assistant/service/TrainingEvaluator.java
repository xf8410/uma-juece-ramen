...
public final class TrainingEvaluator {
    ...
    public String recommend(JSONObject summary){
        JSONObject stats=summary.optJSONObject("stats");int vital=stats!=null?stats.optInt("vital",-1):-1;int max=stats!=null?stats.optInt("max_vital",-1):-1;int motivation=motivation(stats);
        JSONArray trainings=summary.optJSONArray("trainings");if(trainings==null||trainings.length()==0)return "等待训练数据";
        ...
        // Bad/Worst motivation: outing repairs motivation and may be preferable even when vitality is not low.
